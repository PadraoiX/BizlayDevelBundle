...
class GenerateRestCrudService extends DevelService
{
    ...
    protected $createdController = false;
    ...
    public function updateRouter()
    {
        if ($this->createdController) {
            ...
            $nsp = $dto->query->get('BUNDLENAMESPACE');
            $entity = $dto->query->get('ENTITYNAME');
            $targetDir = $dto->query->get('BUNDLEDIR');
            $dsp = DIRECTORY_SEPARATOR;
            $routingFile = $targetDir . 'Resources' . $dsp . 'config' . $dsp . 'routing.yml';
            ...
            $newRoute = "\n" . strtolower(str_replace('\\', '_', $nsp) . '_' . $entity) . ":\n" .
            "  type: rest\n" .
            "  resource: " . $nsp . "\\Controller\\" . $entity . "Controller\n";

            file_put_contents($routingFile, $content . $newRoute);

            echo "Rota adicionada automaticamente ao arquivo " . $routingFile . "\n";
        }
    }
}
